...
import { getTemplateSourceType, readDirWithFileTypes, getRootPath } from '../util'
...
export default class Creator {
  ...
  /** 样板方法 **/
  init () {}
  async write (_callback?: () => void): Promise<void> {}
  async create () {}
  async fetchTemplates(_args?: any): Promise<any> {}
  ...
  async downloadTemplate (templateSource: string, clone?: boolean): Promise<ITemplates[]> {
    const templateRootPath = this.templatePath('')
    const templateSourceType = getTemplateSourceType(templateSource)
    const isFromGit = templateSourceType === 'git'
    const isFromUrl = templateSourceType === 'url'
    ...
    return new Promise<{ error: string; templateSourceName: string } >(async (resolve) => {
      ...
      if (hasSingleTemplate) {
        ...
        const defaultTemplateDesc = isFromGit ? templateSource : ''
