...
export function getUserHomeDir(): string {
  function homedir(): string {
    ...
    if (process.platform === 'win32') return `${env.USERPROFILE || ''}${env.HOMEDRIVE}${env.HOMEPATH || home}`
    if (process.platform === 'darwin') return home || (user ? `/Users/${user}` : '')
    if (process.platform === 'linux') return home || (process.getuid?.() === 0 ? '/root' : user ? `/home/${user}`: '')
    return home
  }
